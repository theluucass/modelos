...
with st.expander("Carga de datos"):
    ...
    for i in range(n):
        ...
        if len(values) != n:
            st.error(f"La fila {i} debe tener exactamente {n} valores.")
...
if st.button("Resolver Ruteo"):
    try:
        assert len(distance_matrix) == len(distance_matrix[0]), "La matriz de distancias debe ser cuadrada."
        if model_type in ["CVRP", "VRPTW"]:
            assert len(demands) == len(distance_matrix), "La cantidad de demandas no coincide con el número de nodos."
            assert len(capacities) == num_vehicles, "Cantidad de capacidades no coincide con número de vehículos."
        if model_type == "VRPTW":
            assert len(time_windows) == len(distance_matrix), "Cantidad de ventanas de tiempo incorrecta."
